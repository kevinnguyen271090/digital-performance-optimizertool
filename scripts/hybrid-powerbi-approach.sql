-- Cấu trúc database kết hợp: Phương án 3 + Power BI
-- 7 ngày cố định + Import + Cache thông minh

-- 1. Bảng lưu dữ liệu 7 ngày cố định (như phương án 3)
CREATE TABLE IF NOT EXISTS dashboard_fixed_7days (
  ...
  dimensions JSONB,
  data_source TEXT NOT NULL DEFAULT 'import', -- 'import', 'api', 'cache'
  last_refresh TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(organization_id, platform, service, date)
);

-- 2. Bảng tổng hợp 7 ngày (như Power BI Aggregations)
CREATE TABLE IF NOT EXISTS dashboard_summary_7days (
  ...
  metrics JSONB NOT NULL, -- Tổng hợp 7 ngày
  last_calculated TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(organization_id, platform, service, period_start)
);

-- 3. Bảng cache cho custom date range (như Power BI DirectQuery cache)
...
CREATE TABLE IF NOT EXISTS refresh_schedule (
  ...
  platform TEXT NOT NULL,
  service TEXT NOT NULL,
  refresh_type TEXT NOT NULL DEFAULT 'daily', -- 'daily', 'hourly', 'custom'
...
-- 6. Function import dữ liệu 7 ngày (kết hợp phương án 3 + Power BI Import)
...
-- 7. Function tính toán tổng hợp 7 ngày (như Power BI Aggregations)
...
-- 8. Function cache custom range (như Power BI DirectQuery cache)
...
-- 10. Function cleanup và maintenance (kết hợp cả hai phương án)
...
BEGIN
  ...
  -- Xóa dữ liệu cũ hơn 7 ngày (như phương án 3)
...
-- 14. Cron jobs
-- SELECT cron.schedule('daily-7days-refresh', '0 2 * * *', 'SELECT hybrid_maintenance();');
-- SELECT cron.schedule('hourly-7days-summary', '0 * * * *', 'SELECT calculate_7days_summary();');
